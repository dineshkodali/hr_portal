...
const FileManager: React.FC<FileManagerProps> = ({ user, onLogActivity }) => {
                ...
                <tr key={file.id} className="hover:bg-slate-50/80 transition-colors group">
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end space-x-2">
                      <button
                        onClick={() => onLogActivity?.('Download File', 'Files', `Downloaded file: ${file.name}`)}
                        className="p-2 text-slate-400 hover:text-accent-600 hover:bg-orange-50 rounded-lg transition-colors"
                        title="Download"
                      >
                        <Download size={16} />
                      </button>
